...
public class PersistenceExample {
	...
	public void run() throws OWLOntologyCreationException {
		...
		// The following code introduces a few changes to the ontology, while the internal state of the classifier is
		// stored in a file.
		// Later, the classifier read back from the file will automatically notice the changes, and incrementally apply
		// them
		...
		try {
			...
			// it is important to provide the ontology here, if we want the classifier to notice the changes that occurred
			// while the
		...
		// Now query both of the classifiers for subclasses of "Pain" class. Both of the classifiers will incrementally
		// update their state, and should print
		// the same information

		System.out.println("[Original classifier] Subclasses of " + pain + ": "
				+ classifier.getSubClasses(pain, true).getFlattened() + "\n");
		System.out.println("[Restored classifier] Subclasses of " + pain + ": "
				+ restoredClassifier.getSubClasses(pain, true).getFlattened() + "\n");
