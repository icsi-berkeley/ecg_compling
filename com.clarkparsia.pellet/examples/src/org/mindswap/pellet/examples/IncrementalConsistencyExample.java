...
/**
 * An example program that incrementally checks consistency through additions to the ABox. The example demonstrates the
 * necessary flags that need to be set, which enable the incremental consistency checking service. Currently the
 * incremental consistency checking service can be used through the Pellet, Jena and OWL APIs. The example loads an
 * ontology, makes ABox changes and incrementally performs consistency checks.
 * 
...
public class IncrementalConsistencyExample {
	...
	public static void runWithOWLAPI() throws Exception {
		...
		// we want a non-buffering reasoner here (a buffering reasoner would not process any additions, until manually
		// refreshed)
