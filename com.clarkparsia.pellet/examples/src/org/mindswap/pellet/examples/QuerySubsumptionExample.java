...
/**
 * Example program to demonstrate the query subsumption (query containment) capabilities of Pellet. Query subsumption
 * service reports if the answers to a query would be contained in the answers of another query. It is similar to
 * concept subsumption service but applies to conjunctive queries. The examples in this sample program show both concept
 * subsumption and concept equivalence services. The examples also show how to get the mapping between query variables
...
public class QuerySubsumptionExample {
	...
	/**
	 * Simple query subsumption similar to standard concept subsumption. Every Male is a Person so query 1 is subsumed by
	 * query 2. The converse is obviously not true.
	 */
	...
	/**
	 * Another example of subsumption. First query asks for all people married to Male individuals which is subsumed by
	 * the second query which asks for all Females.
	 */
	...
	/**
	 * Example showing query equivalence. The subproperty relation between hasFather and hasParent properties would
	 * normally establish subsumption in one way but due to cardinality restrictions defined in the ontology two queries
	 ...
	 * however, The second query uses the variable name ?z instead of the the variable name ?y used in the first query.
	 * The query subsumption algorithm finds the mapping between query variables.
	 */
