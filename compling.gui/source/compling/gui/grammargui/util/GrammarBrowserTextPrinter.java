...
/**
 * Implements a local Grammar Text Printer. The reason is that a different format for an HTML anchor's href is needed
 * for the SWT Link Composite.
 * 
 * Copied from <a href="compling.grammar.ecg#HTMLGrammarPrinter"> compling.grammar.ecg.HTMLGrammarPrinter</a>
 * 
 * @author emok, modified by lucag
 */
public class GrammarBrowserTextPrinter implements ECGGrammarFormatter, GrammarBrowserGUIConstants {

	private static final String ANCHOR_FORMAT = "<a href=\"%s/%s\">%s</a>";

	/**
	 * Generates and HTML link that doesn't make use of attributes for encoding the type (which can be either a
	 * COnstrucion, a Schema, a Block, and so on.
	 * 
	 * @param linkText
