...
public class LBFGSMinimizer implements GradientMinimizer {
	...
	LinkedList inputDifferenceVectorList = new LinkedList(); // double[]
	LinkedList derivativeDifferenceVectorList = new LinkedList(); // double[]
	...
	public double[] minimize(DifferentiableFunction function, double[] initial, double tolerance) {
		...
		for (int iteration = 0; iteration < maxIterations; iteration++) {
			...
			// System.out.println("Iteration "+iteration+" ended with value
			// "+nextValue);
		...
		// System.err.println("LBFGSMinimizer.minimize: Exceeded maxIterations
		// without converging.");
		return guess;
	}
	...
	private void pushOntoList(double[] vector, LinkedList vectorList) { // double[]
	...
	private double[] getInputDifference(int num) {
		...
		return (double[]) inputDifferenceVectorList.get(num);
	}

	private double[] getDerivativeDifference(int num) {
		return (double[]) derivativeDifferenceVectorList.get(num);
	}

	private double[] getLastDerivativeDifference() {
		return (double[]) derivativeDifferenceVectorList.getFirst();
	}

	private double[] getLastInputDifference() {
		return (double[]) inputDifferenceVectorList.getFirst();
	}
