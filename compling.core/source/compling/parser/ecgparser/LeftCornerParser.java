  ...
  public PriorityQueue<List<T>> getBestPartialParses(Utterance<Word, String> utterance) {
    ...
    for (int i = 0; i < utterance.size(); i++) {
      ...
      try {
    	...
    	Set<String> lems = null;
    	if (i + 1 < utterance.size()) {
    		wordform = utterance.getElement(i).getOrthography() + "_" + utterance.getElement(i + 1).getOrthography();
    		
    		try {
				lems = this.morpher.getLemmas(wordform);
    			...
    			wordform = utterance.getElement(i).getOrthography();
        		lems = this.morpher.getLemmas(wordform);
    		}
    	} else {
    		wordform = utterance.getElement(i).getOrthography();
        ...
        // TODO: the multiword expression stuff
	    	try {
