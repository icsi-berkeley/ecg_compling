...
import compling.grammar.unificationgrammar.FeatureStructureUtilities.TexFeatureStructureFormatter;
...
import compling.parser.ecgparser.morph.LeftCornerParserTablesCxn.AnalysisInContextFactory;
...
import compling.parser.ecgparser.morph.LeftCornerParserTablesCxn.ParamFileConstituentExpansionCostTable;
import compling.parser.ecgparser.morph.LeftCornerParserTablesCxn.ParamFileConstituentExpansionCostTableCFG;
import compling.parser.ecgparser.morph.LeftCornerParserTablesCxn.ParamFileConstituentExpansionCostTableFromCounts;
...
public class ECGAnalyzerWithMorph implements compling.parser.Parser<Analysis> {
	private LeftCornerParserWithMorphology<Analysis> parser;
	...
	public ECGAnalyzerWithMorph(Grammar grammar) throws IOException {
	...
	public ECGAnalyzerWithMorph(Grammar ecgGrammar, AnalyzerPrefs prefs) throws IOException {
		...
		if (prefs.getList(AP.GRAMMAR_PARAMS_PATHS).size() > 0) {
			...
			parser = new LeftCornerParserWithMorphology<Analysis>(ecgGrammar, factory, cect);
		}
		else {
			parser = new LeftCornerParserWithMorphology<Analysis>(ecgGrammar, factory);
		}
...
//    needs to be more code here to further process the grammar prefs
	...
	public ECGAnalyzerWithMorph(String prefsFileName) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		ECGAnalyzerWithMorph analyzer = new ECGAnalyzerWithMorph(args[0]);
